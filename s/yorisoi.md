...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 30px | 1.75 | 221px | 15px |
| 見出し | 23px | 1.75 | 63px | 33px |
| 小見出し | 20px | — | 9px | 19px |
| リード | 19px | — | — | — |
| 本文 | 16px | 1.75 | — | — |
| 補助 | 14px | 1.75 | 70px | 8px |

- 余白は margin ではなく**実際に描かれた間隔**。その要素の上端 − ひとつ上の文字要素の下端（下はその逆）で測っている。行間の余りぶんを含む。
- 「—」は見出しに使われていないサイズ。測る相手がないので数字が出ない。
- 上の余白は、セクションの先頭に来る見出しだとセクションの上下余白（76px）を含む。まとまりの中の間隔は「見出しのまとまり」を見る。

- 本文は 16px・行間 1.75。

## 見出しのまとまり

小さいラベルを見出しの上に置く型を 4箇所で使っている（見出し6箇所のうち）。
...
```css
.c-head__label{ font-size:14px; font-weight:500; letter-spacing:0.057em;
  color:#999592; text-transform:uppercase }
.c-head__title{ font-size:23px; line-height:1.75; margin-top:3px }
.c-head__title + *{ margin-top:33px }
```

- ラベルと見出しの間は 3px、見出しと本文の間は 33px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
.c-head__label{ font-size:14px; font-weight:500; letter-spacing:0.057em; color:#999592 }
.c-head__title{ font-size:23px; line-height:1.75; margin-top:3px }
.c-head__title + *{ margin-top:33px }
