...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 40px | 1.35 | 30px | 35px |
| 見出し | 28px | — | — | 40px |
| 本文 | 16px | 1.72 | — | — |
| 補助 | 14px | 1.72 | 340px | 69px |
| 注記 | 13px | — | — | — |

- 余白は margin ではなく**実際に描かれた間隔**。その要素の上端 − ひとつ上の文字要素の下端（下はその逆）で測っている。行間の余りぶんを含む。
- 「—」は見出しに使われていないサイズ。測る相手がないので数字が出ない。
- 上の余白は、セクションの先頭に来る見出しだとセクションの上下余白（180px）を含む。まとまりの中の間隔は「見出しのまとまり」を見る。
...
.c-head__title{ font-size:40px; line-height:1.35; margin-top:30px }
.c-head__title + *{ margin-top:35px }
```

- ラベルと見出しの間は 30px、見出しと本文の間は 35px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
.c-head__label{ font-size:12px; font-weight:700; color:#ffffff }
.c-head__title{ font-size:40px; line-height:1.35; margin-top:30px }
.c-head__title + *{ margin-top:35px }
