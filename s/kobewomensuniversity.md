...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 50px | 1.21 | 5px | 43px |
| 見出し | 24px | 1.75 | 238px | 4px |
| 小見出し | 18px | 1.22 | 197px | 8px |
| 本文 | 15px | 1.22 | 8px | 15px |
| 補助 | 13px | — | — | — |
| 注記 | 12px | 1.22 | 8px | 197px |
...
小さいラベルを見出しの上に置く型を 6箇所で使っている（見出し7箇所のうち）。
...
```css
.c-head__label{ font-size:15px; font-weight:700; letter-spacing:0.05em;
  color:#222222 }
.c-head__title{ font-size:50px; line-height:1.21; margin-top:5px }
.c-head + *{ margin-top:43px }
```

- ラベルと見出しの間は 5px、見出しと本文の間は 43px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
| 8 | 1980px | — | 6カラム・画像あり | 左 | 見出しの下 |
| 9 | 200px | — | 6カラム | 左 | — |

- 全9セクション。
- 使われている面の色: `#eef7fb`（8） / `#000000`（2） / `#ffffff`（1） / `#7fc7e2`（1）
- 見出しは左5／中央3。
...
角丸は 0px だが、**完全な円は別扱い**で 39 箇所ある（48px×15、24px×14、96px×7）。
...
.c-head__label{ font-size:15px; font-weight:700; letter-spacing:0.05em; color:#222222 }
.c-head__title{ font-size:50px; line-height:1.21; margin-top:5px }
.c-head + *{ margin-top:43px }
