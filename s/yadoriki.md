...
文字色は `#3d3535` / `#ffffff` / `#a79b9b` / `#da1517`。
...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 46px | 1.7 | 0px | 95px |
| 見出し | 40px | 1.7 | 192px | 37px |
| 小見出し | 23px | 1.7 | 27px | 7px |
| 本文 | 19px | 1.75 | — | — |
| 補助 | 16px | — | — | — |
| 注記 | 15px | — | — | — |
...
小さいラベルを見出しの上に置く型を 4箇所で使っている（見出し8箇所のうち）。
...
```css
.c-head__label{ font-size:19px; font-weight:400; letter-spacing:0.04em;
  color:#da1517; text-transform:uppercase }
.c-head__title{ font-size:46px; line-height:1.7; margin-top:0px }
.c-head + *{ margin-top:95px }
```

- ラベルと見出しの間は 0px、見出しと本文の間は 95px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
| 6 | 680px | `#fdf3f3` | 5カラム・画像あり | — | 全面 |

- 全6セクション、すべて全幅。中央に寄せた箱を積むのではなく、色面を全幅で切り替えながら進む。
- 使われている面の色: `#ffffff`（10） / `#f7f6f6`（3） / `#fdf3f3`（2） / `#2f2d2d`（1）
- 見出しは左3／中央1。
...
.c-head__label{ font-size:19px; font-weight:400; letter-spacing:0.04em; color:#da1517 }
.c-head__title{ font-size:46px; line-height:1.7; margin-top:0px }
.c-head + *{ margin-top:95px }
