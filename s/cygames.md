...
文字色は `#ffffff` / `#000000` / `#999999` / `#3860be`。
...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 62px | 1.17 | 235px | 31px |
| 見出し | 30px | — | — | — |
| 小見出し | 27px | 1.15 | 110px | 145px |
| リード | 21px | 1.15 | 71px | 37px |
| 本文 | 14px | 1.78 | — | — |
| 補助 | 10px | — | — | — |

- 余白は margin ではなく**実際に描かれた間隔**。その要素の上端 − ひとつ上の文字要素の下端（下はその逆）で測っている。行間の余りぶんを含む。
- 「—」は見出しに使われていないサイズ。測る相手がないので数字が出ない。
- 上の余白は、セクションの先頭に来る見出しだとセクションの上下余白（60px）を含む。まとまりの中の間隔は「見出しのまとまり」を見る。
...
| | PC 1440px | スマホ 390px |
|---|---|---|
| 本文 | 14px / 行間 1.78 | 10px / 行間 1.15 |
| 見出し | 62px | 21px / 行間 1.15 |
| セクションの上下余白 | 60px | 44px |
| 左右の余白 | — | 31px |
| 並びの間隔 | 25px | 16px |

- 本文は 14px → 10px、セクション余白は 60px → 44px（PCの73%）。
...
| 2 | 620px | — | 2カラム・画像あり | 左 | 見出しの下 |
| 3 | 1660px | `#f5f5f5` | 1カラム・画像あり | 右 | 左（50:50） |
| 4 | 440px | `#f5f5f5` | 6カラム・画像あり | 左 | — |
| 5 | 480px | `#f5f5f5` | 1カラム・画像あり | 右 | — |
| 6 | 560px | `#f5f5f5` | 2カラム・画像あり | 右 | 左（27:73） |
| 7 | 420px | — | 4カラム・画像あり | 左 | 右（53:47） |
| 8 | 220px | `#dddddd` | 6カラム・画像あり | — | — |

- 全8セクション。
- 使われている面の色: `#000000`（13） / `#ffffff`（6） / `#f5f5f5`（4） / `#dddddd`（1）
- 見出しは左3／中央1。
- 2カラムの分け方は 50:50 / 27:73 / 53:47。半分ずつには割らない。
...
- 24枚使っている。うち 7 枚は画面いっぱいに置く
- 比率は 3:2（15枚）、16:9（2枚）、21:9（1枚）
- 角丸 0px。切り抜かず四角のまま置く
...
@media (max-width:835px){
  :root{ --fs-body:10px; --section-y:44px; --gap:16px; }
