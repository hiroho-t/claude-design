...
白地に `#edeff0` を大きな面で置く配色。影も枠線もほとんど使わない。本文 18px・行間 1.5、セクション間 92px。
...
:root{
  ...
  --ink: #000000;
  --ink-rev: #ffffff;
  --on: #edeff0;   /* いま乗っている面の上で使う線と文字の色。面ごとに入れ替える */
...
- 主色 `#edeff0` は差し色ではなく**面**で使う。画面の18%を占めている。
...
- `#edeff0` は面として12箇所、文字として0箇所。塗りが主役。ボタンの地にも使う。
...
- 主色 `#edeff0` の面が 2 箇所。地色と主色の面を交互に置くのがリズムのつくり方。
...
.section{ padding:var(--section-y) 0; --on:#edeff0 }
...
- 地色と主色 `#edeff0` の面を全幅で交互に置く。主色は画面の18%を占めるだけ使う。
