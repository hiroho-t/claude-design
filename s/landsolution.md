...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 48px | 1.14 | 25px | 40px |
| 見出し | 24px | 1.2 | 120px | 46px |
| 小見出し | 20px | 1.6 | 81px | 81px |
| リード | 18px | — | 10px | 20px |
| 本文 | 16px | 1 | — | — |
| 補助 | 14px | — | — | — |
| 注記 | 12px | 1.5 | 236px | 309px |
...
```css
.c-head__label{ font-size:18px; font-weight:400;
  color:#2f4243; text-transform:uppercase }
.c-head__title{ font-size:48px; line-height:1.14; margin-top:25px }
.c-head + *{ margin-top:40px }
```

- ラベルと見出しの間は 25px、見出しと本文の間は 40px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
.card{
  ...
  border: 1px solid var(--on);   /* 実測は #1f2529。面によって入れ替える */
  border-radius: 50px;
  padding: 48px 0px;
}
```
...
角丸は 0px だが、**完全な円は別扱い**で 11 箇所ある（24px×8、312px×3）。
アイコンの地・点・装飾に使う。角を丸めないルールと、円のモチーフは両立する。

## 画像

- 18枚使っている。うち 4 枚は画面いっぱいに置く
...
.c-head__label{ font-size:18px; font-weight:400; color:#2f4243 }
.c-head__title{ font-size:48px; line-height:1.14; margin-top:25px }
.c-head + *{ margin-top:40px }

.card{ background:transparent; border:1px solid var(--on);
  border-radius:50px; padding:48px 0px }
