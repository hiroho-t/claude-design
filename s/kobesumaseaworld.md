...
白地に `#0071bc` を文字と細部だけで効かせる配色。影を使って浮かせる。本文 14px・行間 1.65、セクション間 100px。
...
- `#0071bc` は面として30箇所、文字として3箇所。塗りが主役。ボタンの地にも使う。
...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 30px | 1.5 | 30px | 54px |
| 見出し | 22px | — | — | — |
| 小見出し | 20px | 1.23 | 320px | 65px |
| リード | 16px | — | — | — |
| 本文 | 14px | 1.23 | — | 60px |
| 補助 | 12px | — | — | — |
| 注記 | 10px | — | — | — |
...
- セクションの上下余白: 100 / 60 / 80 / 40px（基本は 100px）
- 並びの間隔: 10 / 20 / 30 / 40px
...
| 3 | 560px | — | 1カラム・画像あり | 左 | 右（19:81） |
| 4 | 900px | — | 1カラム・画像あり | — | 全面 |
| 5 | 400px | — | 3カラム・画像あり | — | 全面 |
| 6 | 840px | — | 1カラム・画像あり | 中央 | — |
| 7 | 1000px | — | 1カラム・画像あり | — | 全面 |
| 8 | 1200px | — | 1カラム・画像あり | 中央 | 見出しの下 |
| 9 | 1020px | `#c8e8ef` | 2カラム・画像あり | 中央 | 見出しの下 |
| 10 | 560px | — | 3カラム・画像あり | 中央 | 見出しの下 |
| 11 | 900px | — | 1カラム・文字だけ | — | — |

- 全11セクション、すべて全幅。中央に寄せた箱を積むのではなく、色面を全幅で切り替えながら進む。
- 使われている面の色: `#0071bc`（23） / `#002b60`（3） / `#ffffff`（2） / `#c8e8ef`（2）
- 見出しは左1／中央4。
...
角丸は 0px だが、**完全な円は別扱い**で 41 箇所ある（40px×14、72px×8、120px×6）。
アイコンの地・点・装飾に使う。角を丸めないルールと、円のモチーフは両立する。

## 画像

- 52枚使っている。うち 5 枚は画面いっぱいに置く
- 比率は 3:2（18枚）、1:1（12枚）、16:9（9枚）
- 角丸 0px。切り抜かず四角のまま置く
...
- 余白 100px と行間 1.65 を先に決めてから中身を入れる。
