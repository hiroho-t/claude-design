...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 72px | 1 | 123px | 60px |
| 見出し | 32px | — | — | 30px |
| 小見出し | 18px | 1.5 | 40px | 10px |
| 本文 | 16px | 1.5 | 78px | 5px |
| 補助 | 15px | — | — | — |
| 注記 | 14px | — | — | 232px |

- 余白は margin ではなく**実際に描かれた間隔**。その要素の上端 − ひとつ上の文字要素の下端（下はその逆）で測っている。行間の余りぶんを含む。
- 「—」は見出しに使われていないサイズ。測る相手がないので数字が出ない。
- 上の余白は、セクションの先頭に来る見出しだとセクションの上下余白（124px）を含む。まとまりの中の間隔は「見出しのまとまり」を見る。

- 本文は 16px・行間 1.75。

## レイアウト

- コンテンツ幅: 最大 1060px／読ませる段は 672px
...
.hero{ min-height:6640px; display:grid; align-content:center }
