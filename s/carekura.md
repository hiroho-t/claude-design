...
文字色は `#18181b` / `#34a9ee` / `#000000` / `#ffffff`。
...
- `#34a9ee` は面11箇所・文字29箇所を行き来する。ボタンの地にも使う。枠線にも4箇所。
...
| 用途 | サイズ | 行間 | 上の余白 | 下の余白 |
|---|---|---|---|---|
| 大見出し | 64px | 2.4 | 144px | 268px |
| 見出し | 48px | 1.3 | 257px | 100px |
| 小見出し | 44px | — | 302px | 218px |
| リード | 22px | 1.5 | 5px | 301px |
| 本文 | 16px | 2.2 | — | — |
| 補助 | 15px | 2 | 7px | 176px |
| 注記 | 14px | — | — | — |
...
- 上の余白は、セクションの先頭に来る見出しだとセクションの上下余白（128px）を含む。まとまりの中の間隔は「見出しのまとまり」を見る。
...
```css
.c-head__label{ font-size:11px; font-weight:400; letter-spacing:0.073em;
  color:#ffffff }
.c-head__title{ font-size:20px; line-height:1.7; margin-top:14px }
.c-head__title + *{ margin-top:67px }
```

- ラベルと見出しの間は 14px、見出しと本文の間は 67px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
.c-head__label{ font-size:11px; font-weight:400; letter-spacing:0.073em; color:#ffffff }
.c-head__title{ font-size:20px; line-height:1.7; margin-top:14px }
.c-head__title + *{ margin-top:67px }
