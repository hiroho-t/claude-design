...
文字色は `#080808` / `#464543` / `#979797` / `#b49646`。
...
- `#b49646` は文字色として36箇所で使うのが主。面としては9箇所しかないが、1枚が大きく画面の25%を占める。ボタンの地にも使う。枠線にも1箇所。
...
- 上の余白は、セクションの先頭に来る見出しだとセクションの上下余白（244px）を含む。まとまりの中の間隔は「見出しのまとまり」を見る。

- 本文は 14px・行間 1。

## 見出しのまとまり

小さいラベルを見出しの上に置く型を 3箇所で使っている（見出し6箇所のうち）。
...
```css
.c-head__label{ font-size:10px; font-weight:400;
  color:#ffffff; text-transform:uppercase }
.c-head__title{ font-size:18px; margin-top:7px }
.c-head + *{ margin-top:133px }
```

- ラベルと見出しの間は 7px、見出しと本文の間は 133px。ラベル側を詰めて、本文側を空ける。この差がまとまりを作っている。
...
| | PC 1440px | スマホ 390px |
|---|---|---|
| 本文 | 14px / 行間 1 | 10px / 行間 1 |
| セクションの上下余白 | 244px | 108px |
| 左右の余白 | — | 20px |
| 並びの間隔 | 16px | —px |

- 本文は 14px → 10px、セクション余白は 244px → 108px（PCの44%）。
...
| # | 高さ | 地色 | 中身 | 見出し | 画像 |
|---|---|---|---|---|---|
| 1 | 1140px | `#080808` | ヒーロー（画像） | 中央 | 見出しの下 |
| 2 | 2360px | `#b49646` | 1カラム・画像あり | 中央 | — |
| 3 | 940px | — | 4カラム・画像あり | 中央 | — |
| 4 | 700px | — | 1カラム・画像あり | 左 | — |
| 5 | 400px | — | 2カラム・画像あり | 左 | — |
...
.c-head__label{ font-size:10px; font-weight:400; color:#ffffff }
.c-head__title{ font-size:18px; margin-top:7px }
.c-head + *{ margin-top:133px }
...
@media (max-width:569px){
  :root{ --fs-body:10px; --section-y:108px; }
