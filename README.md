...
**文言・写真・ロゴは持ち出さない。**（`data/*.measure.json` だけは、どこを測ったかを示すために
相手のセレクタが入っている。`s/*.md` には入らない）
...
| `data/<slug>.measure.json` | startwith-site 側が別の道具で測った生の値（270件）。余白の段・行長・グリッド・タップ領域など |
| `data/_measure-index.json` | その24指標の分布（中央値・四分位）。自社がどこに居るかを見るためのもの |
